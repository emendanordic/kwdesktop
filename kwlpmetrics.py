...
import kwlpconstants as KWLP_CONST
...
class KwLocalProjectMetrics:
    def __init__(self, logger, project_dir, metrics_report, metrics_ref):
        logger.debug("Initialising KwLocalProjectMetrics class")
        self.logger = logger
        ...
        self.logger.debug("Initialisation of KwLocalProjectMetrics complete")
    ...
    def validate_metrics_dat_files(self):
        if not (os.path.exists(self.metric_dat) and
            ...
            sys.exit("Could not find .dat files in {0}".format(self.metrics_dir))
